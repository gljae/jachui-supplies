...
## E. 최종 체크리스트 — Phase 10 점검 결과

- [x] 375px에서 가로 스크롤 없음, 모든 버튼 44px 이상
      — 홈·상세·통계·설정 네 화면에서 `scrollWidth === innerWidth`, 44px 미만 타겟 0개.
        설정의 체크박스만 20px이지만 라벨(309×44)이 감싸고 있어 실제 터치 영역은 44px다
- [x] 모든 IndexedDB 호출이 try-catch로 감싸이고 실패가 화면에 뜬다 — `guard()` + Toast
- [x] 삭제는 전부 확인 모달을 거친다 (이력 / 품목 / 초기화)
- [x] 메인 화면에 영수증·그래프가 없다
- [x] 통화 `₩` + 천 단위 구분, 날짜 `YYYY.MM.DD`
- [x] 단위 그룹 혼재 품목에서 용량 토글이 비활성이다 — `calc.test.ts`로 고정
- [x] `npm test` 통과(112개), `npm run build` 경고 없이 성공
      — 코드 분할 후 500KB 초과 경고가 사라졌다
- [x] 크림색+세리프+테라코타류 템플릿 기본값을 쓰지 않았다

### Phase 10에서 처리한 미결 항목

| 항목 | 처리 |
|---|---|
| 번들 500KB 초과 경고 | 차트 화면을 `lazy`로 분리. 초기 로드 500KB+ → 295KB, recharts는 357KB 별도 청크 |
| 자정을 넘겨도 날짜가 안 바뀜 | `useToday()` — 자정과 탭 복귀 시 갱신하고, 같은 날이면 참조를 유지해 `useMemo`를 깨지 않는다 |
| 시트가 열린 채 뒤로가기 (G8) | 히스토리에 표시용 항목을 얹어 뒤로가기가 시트만 닫게 했다 |
| 다중 탭 유령 데이터 (G2) | `BroadcastChannel`로 변경을 알리고 각 탭이 스스로 다시 읽는다 |
| 타입 칩과 토글 칩이 구분 안 됨 | 타입은 붙은 세그먼트(`role="radiogroup"`)로, 나머지는 떨어진 토글 칩으로 |
| 확인 모달 입력이 남음 | 본문을 별도 컴포넌트로 분리해 열 때마다 초기화. 취소 후 다시 열면 확인 문구가 비어 있다 |
| Fast Refresh 무효화 | `cardStats`를 `lib/cardStats.ts`로 옮겼다. 컴포넌트 파일의 비컴포넌트 export가 개발 중 모듈 그래프를 깨뜨려 이번 작업에서 여러 번 서버를 재시작해야 했다 |

### 남긴 것 (의도적)

- `useData` / `useToast` / `depletionLabel`이 각자의 컴포넌트 파일에 함께 있어 lint가
  `only-export-components`를 경고한다. 훅을 프로바이더 옆에 두는 건 흔한 배치이고,
  파일 세 개를 더 만들 만한 이득이 없다고 봤다.
- 영수증 objectURL을 만드는 두 곳에서 `set-state-in-effect` 경고가 난다. URL 생성과
  해제를 짝지으려면 effect가 맞고, 렌더 중에 만들면 해제 시점을 잡을 수 없다.
